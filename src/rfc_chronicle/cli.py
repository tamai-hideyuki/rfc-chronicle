...
def fetch(save: bool) -> None:
    ...
    try:
        metas = rfc_client.fetch_metadata(save=False)
        click.echo(f"Fetched {len(metas)} records.")
        if not save:
            return
...
@click.option("-o", "--output", type=click.Choice(["json", "csv", "md"]), default="md", show_default=True)
def show(number: int, output: str) -> None:
    """指定 RFC の詳細を表示・エクスポート"""
    try:
        meta = next(
            m for m in rfc_client.fetch_metadata(save=False)
...
@click.option("--vectors", "-v", default="data/vectors.npy", show_default=True, help="入力ベクトル (.npy)")
@click.option("--index", "-i", default="data/faiss_index.bin", show_default=True, help="出力インデックス (.bin)")
@click.option("--update", "-u", is_flag=True, help="既存インデックスへ追加登録")
@click.option("--type", "-t", type=click.Choice(["flat", "ivf", "hnsw"]), default="flat", show_default=True, help="インデックスタイプ")
def build_faiss(vectors: str, index: str, update: bool, type: str) -> None:
    ...
    save_index(idx, idx_path)
    click.echo(f"Index saved → {idx_path}")
